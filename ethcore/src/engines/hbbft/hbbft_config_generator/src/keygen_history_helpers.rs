...
use ethkey::{public_to_address, Address, Public, Secret};
...
impl SecretKey for KeyPairWrapper {
    ...
    fn decrypt(&self, ct: &[u8]) -> Result<Vec<u8>, Self::Error> {
        ethkey::crypto::ecies::decrypt(&self.secret, b"", ct)
            .map_err(|_| ethkey::Error::InvalidSecret)
    }
}
...
pub fn key_sync_history_data(
    ...
    for id in ids {
        ...
        data.staking_addresses
            .push(format!("{:?}", Address::from(staking_counter)));
        staking_counter += 1;
        data.public_keys
            .push(format!("{:?}", enodes.get(id).unwrap().public));
        data.ip_addresses.push(format!("{:?}", H128::from(1u64)));
